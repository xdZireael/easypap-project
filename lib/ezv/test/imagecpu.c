...
int main (int argc, char *argv[])
{
  ...
  ctx[1] =
      ezv_ctx_create (EZV_CTX_TYPE_IMG2D, "CPU view", SDL_WINDOWPOS_UNDEFINED,
  ...
  // Some initial colors…
  ezv_set_cpu_color_2D (ctx[1], 0, img.width, 0, img.height / 2,
                        ezv_rgb (255, 255, 0));
  ezv_set_cpu_color_2D (ctx[1], 0, img.width, img.height / 2, img.height / 2,
                        ezv_rgb (255, 0, 0));
